...
public function export_csv() {
    ...
    if ($type === 'leads') {
        ...
        if (current_user_can('client')) {
            $where[] = "l.client_id = " . get_current_user_id();
        } else {
            if (!empty($_GET['client_id'])) {
                $where[] = "l.client_id = " . absint($_GET['client_id']);
            }
        }

        // Filters
        if (!empty($_GET['from'])) {
            $where[] = "DATE(l.created_at) >= '" . esc_sql($_GET['from']) . "'";
        }
        if (!empty($_GET['to'])) {
            $where[] = "DATE(l.created_at) <= '" . esc_sql($_GET['to']) . "'";
        }
        if (!empty($_GET['campaign_id'])) {
            $where[] = "l.campaign_id = " . absint($_GET['campaign_id']);
        }
        if (!empty($_GET['ad_name'])) {
            $where[] = "l.ad_name = " . absint($_GET['ad_name']);
        }
        if (!empty($_GET['adset'])) {
            $where[] = "l.adset = " . absint($_GET['adset']);
        }
    ...
    // Other exports (daily tracker, campaign detail, etc.) can be added below...
}
