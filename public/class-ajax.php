...
class PPC_CRM_Ajax
{
    ...
    public function get_leads()
    {
        ...
        $where = $client_id
            ? $wpdb->prepare("WHERE client_id = %d", $client_id)
            : "";
        ...
        $total = (int) $wpdb->get_var(
            "SELECT COUNT(*) FROM {$wpdb->prefix}lcm_leads $where"
        );
    ...
    public function create_lead()
    {
        ...
        if (!$data["campaign_id"]) {
            wp_send_json_error(
                ["msg" => "Please pick a valid campaign or adset"],
                400
            );
        }
    ...
    public function delete_campaign()
    {
        ...
        $total = (int) $wpdb->get_var(
            "SELECT COUNT(*) FROM {$wpdb->prefix}lcm_campaigns"
        );
        wp_send_json_success(["total" => $total]);
    }
}
