...
jQuery(function ($) {
  ...
  const opts = (arr, cur="") =>
    "<option value=''></option>" +
    arr.map(o => `<option ${o == cur ? "selected" : ""}>${o}</option>`).join("");
    ...
    cols.forEach(([f, _label, type, opt]) => {
      ...
      if (type === "action") {
        ...
        html += `<td><select class="form-select form-select-sm"
                             data-name="${f}"${dis}>${opts(opt, v)}</select></td>`;
      }
      else if (type === "date")    {
  ...
  $tbody.on("click", ".save-camp", function () {
    ...
    if (!data.adset) { alert("Adset required"); return; }
  ...
  $tbody.on("input", "[data-name=leads]", function () {
    ...
    $tr.find("td").eq(16).text(Math.max(0, leads - c - n - r));
  });
