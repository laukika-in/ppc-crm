...
private function sanitize_array( $array ) : array {
    ...
    // Recount logic including not_relevant
    public function recount_campaign_counters( $adset ) {
        global $wpdb;
        $rows = $wpdb->get_results($wpdb->prepare(
            "SELECT attempt_type,COUNT(*) AS qty FROM {$wpdb->prefix}lcm_leads WHERE adset=%s GROUP BY attempt_type",
            $adset
        ), ARRAY_A);
        $notRelevant = $relevant = $notConnected = 0;
        foreach($rows as $r){
            if($r['attempt_type']==='Connected:Relevant')        $relevant    = (int)$r['qty'];
            if($r['attempt_type']==='Connected:Not Relevant')    $notRelevant = (int)$r['qty'];
            if($r['attempt_type']==='Not Connected')              $notConnected= (int)$r['qty'];
        }
        $connected = $relevant + $notRelevant;
        $scheduled = (int)$wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}lcm_leads WHERE adset=%s AND attempt_status='Store Visit Scheduled'",
            $adset
        ));
        $shown     = (int)$wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}lcm_leads WHERE adset=%s AND store_visit_status='Show'",
            $adset
        ));
        $leads_total = (int)$wpdb->get_var($wpdb->prepare(
            "SELECT leads FROM {$wpdb->prefix}lcm_campaigns WHERE adset=%s LIMIT 1",
            $adset
        ));
        $na = max(0, $leads_total - $connected - $notConnected - $relevant);
        $wpdb->update(
            ...
            [
                ...
                'relevant'              => $relevant,
                'scheduled_store_visit' => $scheduled,
                'store_visit'           => $shown,
                'not_available'         => $na,
            ],
            [ 'adset' => $adset ],
    ...
    public function title_placeholder( $text, $post ) {
        if ( $post->post_type === 'lcm_campaign' ) return 'Campaign Title';
        if ( $post->post_type === 'lcm_lead' )     return 'Lead Title';
        return $text;
    }
